...
UserAdapter.method('saveUserSuccessCallBack', function(callBackData,serverData) {
	this.showMessage("Create User","An email has been sent to "+callBackData['email']+" with a temporary password to login to Spark HRIS.");
	this.get([]);
});
...
UserAdapter.method('save', function() {
	...
	console.log(validator.checkValues);
